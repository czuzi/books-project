...
public interface ShelvedBookRepository extends JpaRepository<ShelvedBook, Long> {
    ...
    @Query("select sb from ShelvedBook sb where (:searchTerm is null or CONCAT(lower(sb.book.author), ' ',lower(sb.book.title)) like concat('%', lower(:searchTerm), '%')) and sb.shelf.id = :shelfId")
